...
export function VMIEntryPage() {
  ...
  useEffect(() => {
    const pbw = calculatePBWKg(formData.heightCm, formData.sex);
    if (pbw == null) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Block submit if PBW invalid (auto-only)
    if (!formData.predictedBodyWeight || formData.predictedBodyWeight <= 0) {
      setAlerts(prev => [
        '⚠️ No se puede calcular PBW. Completá talla y sexo con valores válidos antes de guardar.',
        ...prev
      ]);
      return;
    }
    ...
    addVMIRecord({
      ...
      mechanicalPower: calculations.mechanicalPower,
  ...
  const canSave = formData.predictedBodyWeight > 0;
      ...
      <main className="max-w-2xl mx-auto p-4 pb-24">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <VMIField
                    ...
                    onChange={e => setFormData({ ...formData, tidalVolumeSet: Number(e.target.value) })}
                    required
                  />
                  <VMIField
                ...
                <div className="grid grid-cols-2 gap-4">
                  <VMIField
                    ...
                    type="number"
                    value={formData.peep}
                    onChange={e => setFormData({ ...formData, peep: Number(e.target.value) })}
                    required
                  />
                  <VMIField
                    label="FiO₂"
                    ...
                    type="number"
                    value={formData.fio2}
                    onChange={e => setFormData({ ...formData, fio2: Number(e.target.value) })}
                    required
                  />
                </div>
                ...
                {formData.controlVariable === 'volume' && (
                  <VMIField
                    ...
                    onChange={e => setFormData({ ...formData, peakPressure: Number(e.target.value) })}
                  />
                )}
              </div>
            </CollapsibleSection>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <VMIField
                    ...
                    onChange={e => setFormData({ ...formData, tidalVolumeExpired: Number(e.target.value) })}
                    required
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <VMIField
                    ...
                    onChange={e => setFormData({ ...formData, plateauPressure: Number(e.target.value) })}
                    required
                  />
                  <VMIField
                    label="PEEP"
                  ...
                  <>
                    <div
                      className={`p-6 rounded-xl text-center ${
                        mpResult.value! > 17 ? 'bg-red-50' : mpResult.value! > 12 ? 'bg-yellow-50' : 'bg-green-50'
                      }`}
                    >
                      <div className="flex items-center justify-center gap-2 mb-2">
                        <ZapIcon
                          className={`w-6 h-6 ${
                            mpResult.value! > 17 ? 'text-red-600' : mpResult.value! > 12 ? 'text-yellow-600' : 'text-green-600'
                          }`}
                        />
                      ...
                      <div className="text-sm mt-3 font-medium">
                        {mpResult.value! > 17 && 'Alto riesgo de VILI'}
                        {mpResult.value! > 12 && mpResult.value! <= 17 && 'Zona gris - Monitoreo estrecho'}
                        {mpResult.value! <= 12 && 'Rango seguro'}
                      </div>
                    </div>
          ...
          <CollapsibleSection title="4) Oxigenación y Equilibrio Ácido-Base" subtitle="Gasometría y parámetros de intercambio">
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <VMIField label="SpO₂" unit="%" type="number" value={formData.spo2} onChange={e => setFormData({ ...formData, spo2: Number(e.target.value) })} />
                <VMIField label="PaO₂" educationKey="pfRatio" unit="mmHg" type="number" value={formData.pao2} onChange={e => setFormData({ ...formData, pao2: Number(e.target.value) })} />
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <VMIField label="PaCO₂" unit="mmHg" type="number" value={formData.paco2} onChange={e => setFormData({ ...formData, paco2: Number(e.target.value) })} />
                <VMIField label="pH" type="number" step="0.01" value={formData.ph} onChange={e => setFormData({ ...formData, ph: Number(e.target.value) })} />
                <VMIField label="HCO₃" educationKey="hco3" unit="mEq/L" type="number" step="0.1" value={formData.hco3} onChange={e => setFormData({ ...formData, hco3: Number(e.target.value) })} />
              </div>
            </div>
          </CollapsibleSection>
              ...
              {(formData.weaningStatus === 'candidate' || formData.weaningStatus === 'sbt-trial') && (
                <>
                  ...
                  {formData.sbtPerformed && (
                    <>
                      <VMIField label="Tipo de SBT" placeholder="Ej: Tubo en T, PS 5-7" value={formData.sbtType} onChange={e => setFormData({ ...formData, sbtType: e.target.value })} />
                      ...
                      {formData.sbtResult === 'failure' && (
                        <VMIField label="Motivo de falla" placeholder="Ej: Taquipnea, desaturación" value={formData.sbtFailureReason} onChange={e => setFormData({ ...formData, sbtFailureReason: e.target.value })} />
                      )}
                    </>
                  )}
                </>
