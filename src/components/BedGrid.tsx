...
export function BedGrid({
  ...
  return <div className="grid grid-cols-2 gap-4">
      ...
      if (isEmpty) {
        ...
        return <button key={bedNumber} onClick={() => onBedClick(bedNumber)} className="group min-h-[140px] rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 p-4 flex flex-col items-center justify-center gap-3 transition-all active:scale-95 hover:border-blue-400 hover:bg-blue-50">
      ...
      return <button key={bedNumber} onClick={() => onBedClick(bedNumber, patient)} className={`min-h-[140px] rounded-2xl border-2 p-4 flex flex-col gap-3 transition-all active:scale-95 ${colorClass}`}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                <UserIcon className="w-5 h-5" />
                <span className="text-sm font-semibold">Cama {bedNumber}</span>
              </div>
